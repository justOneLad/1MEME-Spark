...
`launchFeeWallet` is a platform-level address that receives the launch fee for every token deployed, paid as raw native ETH/BNB. It is set in the constructor and updatable by the owner via `setLaunchFeeWallet`, including back to `address(0)` — when it's `address(0)`, the fee is sent to `owner` instead (see `_launchFeeRecipient()`), so there's always a valid recipient without forcing a dedicated wallet to be configured up front.
...
If a token's original team goes dark, anyone can apply to have `feeWallet` reassigned — subject
to platform owner approval, so this is not a self-service transfer:

```solidity
applyForCTO(address token, address newFeeWallet) payable   // anyone; must pay >= ctoFee
approveCTO(address token)                                  // owner only; reassigns feeWallet
rejectCTO(address token)                                   // owner only; clears without reassigning
setCTOFee(uint256 fee)                                      // owner only; default 0.1 ether
setCTOFeeWallet(address wallet)                             // owner only; address(0) = owner
```

`ctoFee` is paid to `ctoFeeWallet` **at application time, win or lose** — same as the launch fee,
since this is an admin/anti-spam fee rather than a trading fee, so it doesn't go through the
`platformWallet` split that `claimFees()` uses. `ctoFeeWallet` defaults to `owner` when unset
(`address(0)`), same pattern as `launchFeeWallet`. It's a flat deterrent, not a refundable
deposit. A later application for the same token overwrites an earlier pending one (the earlier
applicant's
fee is not refunded). Applying alone changes nothing; only `approveCTO` actually moves `feeWallet`.
...
constructor(
    ...
    address launchFeeWallet_,    // Platform wallet that receives per-launch fees (native ETH/BNB); address(0) = owner
