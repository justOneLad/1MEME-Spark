...
//
// Shared bounded-fallback instant-buy routing engine, inherited by both
// SparkLauncher and SparkGoLauncher.
...
enum RouteShape { V2_STYLE, V3_STYLE, V4_STYLE, INFINITY_STYLE }
...
abstract contract SparkRouting {
    ...
    function _setRoutes(address quoteToken_, Route[] calldata routes_) internal {
        ...
        external payable returns (uint256 amountOut)
    {
        ...
        if (route_.shape == RouteShape.V2_STYLE) {
            ...
            amountOut = _swapInfinityStyle(route_, quoteToken_, amountIn_, minOut_, recipient_);
        }
    }
    ...
    {
        uint256[] memory amounts = IUniswapV2Router(route_.router).swapExactETHForTokens{value: amountIn_}(
            minOut_, route_.path, recipient_, block.timestamp
        );
    ...
    function _swapInfinityStyle(Route calldata route_, address quoteToken_, uint256 amountIn_, uint256 minOut_, address recipient_)
        private returns (uint256 amountOut)
    {
        amountOut = _executeInfinitySwap(route_.singleton, route_.poolLogic, route_.hook, route_.fee, route_.parameters, address(0), quoteToken_, amountIn_, minOut_, recipient_);
    }

    function _executeInfinitySwap(
        address vault_,
        address poolLogic_,
        address hook_,
        uint24  fee_,
        ...
        _cbExpected = vault_;
        bytes memory result = IVaultSwap(vault_).lock(
            abi.encode(poolLogic_, hook_, fee_, parameters_, currencyIn_, currencyOut_, amountIn_, minOut_, recipient_)
        );
        _cbExpected = address(0);
        amountOut = abi.decode(result, (uint256));
    }
    ...
    function lockAcquired(bytes calldata data) external returns (bytes memory) {
        if (_cbExpected == address(0) || msg.sender != _cbExpected) revert Unauthorized();
        (address poolLogic_, address hook_, uint24 fee_, bytes32 parameters_, address currencyIn_, address currencyOut_, uint256 amountIn_, uint256 minOut_, address recipient_) =
            abi.decode(data, (address, address, uint24, bytes32, address, address, uint256, uint256, address));
        ...
        InfinityPoolKey memory key = InfinityPoolKey({
            ...
            currency1:   zeroForOne ? currencyOut_ : currencyIn_,
            hooks:       hook_,
            poolManager: poolLogic_,
            fee:         fee_,
            parameters:  parameters_
        });
        int256 delta = ICLPoolManagerSwap(poolLogic_).swap(
            key,
            InfinitySwapParams({
                zeroForOne:        zeroForOne,
                amountSpecified:   -int256(amountIn_),
        ...
        if (currencyIn_ == address(0)) {
            IVaultSwap(msg.sender).settle{value: amountIn_}();
        } else {
            IVaultSwap(msg.sender).sync(currencyIn_);
            _safeTransfer(currencyIn_, msg.sender, amountIn_);
            IVaultSwap(msg.sender).settle();
        }
        IVaultSwap(msg.sender).take(currencyOut_, recipient_, amountOut);
        return abi.encode(amountOut);
    }
