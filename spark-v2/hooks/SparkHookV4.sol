...
contract SparkHookV4 {
        ...
        external returns (bytes4, int128)
    {
        ...
        if (!isExactInputSell || nativeDelta <= 0) return (this.afterSwap.selector, int128(0));

        uint256 feeCut = uint256(uint128(nativeDelta)) * HOOK_FEE_BPS / 10_000;
        if (feeCut == 0) return (this.afterSwap.selector, int128(0));

        accruedFees[poolId] += feeCut;
        IV4PoolManagerMinimal(msg.sender).take(address(0), address(this), feeCut);
    ...
    function claimFees(bytes32 poolId) external {
        ...
        if (amount > 0) {
            ...
            _pay(platformWallet, platformCut);
            _pay(info.creator, creatorCut);
        }
